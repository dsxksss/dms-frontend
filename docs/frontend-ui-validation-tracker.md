...
- Dataset preview high-performance table: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/datasets/019f12e9-f5fb-7fb0-aa98-e3d36aa5bb7c` verified preview always renders one `canvas[role="grid"]` with no standard/high-performance toggle, no DOM `<table>`, page size `200`, header sort clicks working without runtime errors, column resize increasing canvas width from `1380` to `1500` and enabling horizontal scroll, while `/registry` remains DOM-only with no high-performance table entry; no browser console errors were reported.
