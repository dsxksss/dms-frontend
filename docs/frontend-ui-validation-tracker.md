...
- Audit focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/audit` verified first `查看变更` dialog open/close, `查看原始 JSON` expansion inside the change dialog, pagination `下一页` to page 2 with `上一页` enabled, and `上一页` returning to page 1 with `上一页` disabled again; no browser console errors were reported.
