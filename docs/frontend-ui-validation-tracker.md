...
- Registry high-performance table mode: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/registry` verified the record table exposes `标准表格` / `高性能表格`; small datasets default to the existing DOM table, manual high-performance mode renders one `canvas[role="grid"]`, switches the page-size selector to `200`, supports row click to open the record drawer, supports header sort clicks without runtime errors, supports dragging a column edge to grow the canvas width and enable horizontal scroll, and switches back to the DOM table with `12` rows per page; no browser console errors were reported.
...
- Dataset focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/datasets` verified `新建数据集` open/cancel, list-row `更多` menu, list delete confirmation/cancel, clickable dataset row to detail, detail preview search (`ADC`, `001`, `ADC-001`, `160`), detail tabs `预览` / `版本` / `溯源` / `协作者`, detail `更多` menu, detail edit open/cancel, detail delete confirmation/cancel, and sidebar return to the dataset list; no browser console errors were reported. `导出 CSV` was clicked twice and produced no console error, but the in-app browser did not expose a download event for the blob-anchor download path, so file-save behavior still needs a manual/browser-download-capable check if required.
- Notebook focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/notebook` verified selecting the existing `未命名记录`, view mode buttons `编辑` / `并排` / `预览`, the `显示已归档` toggle round trip, Markdown toolbar buttons `加粗` / `斜体` / `一级标题` / `二级标题` / `项目符号` / `编号列表` / `引用` / `行内代码` / `链接` / `表格` inserting local editor content, reload discarding the unsaved toolbar test content without persistence, record `更多` menu, and delete confirmation/cancel; no browser console errors were reported. The `归档` menu item was not clicked because it immediately mutates record state without an intermediate confirmation.
- Project settings focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/members` verified `添加成员` dialog open/close with invite send disabled at 0 selections, member `移除` confirmation/cancel with the member still present, first unit `编辑` entering draft mode and `取消`, first unit `删除` confirmation/cancel with the unit still present, and first `管理授权` dialog open/close without toggling any grant checkboxes; no browser console errors were reported. `归档`, visibility buttons, member role changes, and grant checkboxes were not clicked because they mutate project/member/grant state immediately.
- Files focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/files` verified `新建文件夹` dialog open/close with empty-name submit disabled, `上传文件` visible and enabled, root `文件` breadcrumb/resource tree presence, dataset row action menu, `打开数据集` navigation to the `ADC dataset` detail and return to files, and dataset delete confirmation/cancel with `ADC dataset` still present; no browser console errors were reported. Real upload/file-picker selection was intentionally not executed to avoid transmitting a local file, and file-specific row actions were unavailable in the current fixture because the files page currently shows `文件 0`; the previous full file-workflow baseline remains the upload/action coverage source.
- Audit focused button regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/audit` verified folded consecutive audit groups, full group counts such as `连续 10 次` / `连续 39 次`, compact sub-rows without leading action icons, child-row `查看变更` dialog open/close, `查看原始 JSON` expansion inside the change dialog, unified pagination defaulting to 12 visible rows/groups per page, page-size selection to 24 rows/groups per page, pagination `下一页` to page 2 with `上一页` enabled, and `上一页` returning to page 1 with `上一页` disabled again; no browser console errors were reported.
- Project overview and workspace shell focused regression: `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640` verified overview statistics/type distribution/recent activity rendering, sidebar navigation links for `药物资产` / `数据资产` / `数据集` / `实验记录本` / `文件` / `操作记录` / `设置`, breadcrumb return to overview, global search dialog open/ESC close without navigation, language toggle to English and back to Chinese, account menu open/ESC close, account menu `设置` navigation to `/settings`, `← 返回项目列表` navigation to `/projects`, and return to the project overview; no browser console errors were reported. The account menu `退出登录` item was intentionally not clicked.
...
- Real upload/file-picker selection on the files page was not performed during focused button validation without an explicit file to upload.
- Account-menu logout was not clicked during project shell focused validation because it ends the current app session.
...
Expected: Consecutive audit operations are folded for easier scanning; expanded child rows look visually subordinate and do not show a leading action icon. Pagination applies to visible folded rows/groups, defaults to 12 per page, and supports 12 / 24 / 48 rows per page through the shared pagination component.
Actual: The original audit list was flat. The first fold implementation grouped only the current raw page and reused the full-size row treatment for children.
Fix: Added project-audit grouping for adjacent entries by actor/action/entity type within a two-minute window; project audit now fetches the raw audit list for grouping and paginates the grouped result in the UI; compact child rows are indented, smaller, and omit the leading action icon. `Pagination` now exposes the shared 12 / 24 / 48 page-size selector, clamps page offsets, defaults list pages to 12, and is reused by audit, registry, dataset preview, org registry, and tenant lists.
Verification: `npm run build` passed. Browser verified `/projects/019f03a2-cce1-7df3-a08c-ffdfdeae1640/audit`: default `每页行数` is `12`; page 1 shows 12 visible rows/groups with folded counts such as `连续 10 次`; expanding a group shows child rows without `Activity` icons and keeps child `查看变更` working; selecting `24` changes pagination to two pages; page 2 shows full folded counts such as `连续 39 次`; next/previous button disabled states are correct; no browser console errors were reported.
Commit: frontend this commit
